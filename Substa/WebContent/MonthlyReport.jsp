...
<jsp:useBean id="monthlyReport" type="java.util.ArrayList<substa.beans.SalesRecord>" scope="session"/>
<jsp:useBean id="month" type="java.lang.String" scope="session"/>
				...
				<table class="table table-hover">
					...
					<tbody>
						...
						<tr onclick="showAuctionDetail(<%=i%>)">
							<th scope="row"><%=i %></th>
							<td><%=monthlyReport.get(i).getAuctionID() %></td>
							<td><%=monthlyReport.get(i).getBuyerID() %></td>
							<td><%=monthlyReport.get(i).getSellerID() %></td>
							<td><span class="glyphicon glyphicon-usd" aria-hidden="true"></span> <%=monthlyReport.get(i).getPrice()%></td>							
							<% SimpleDateFormat before = new SimpleDateFormat("yyyy-m-dd hh:mm:ss");
						   		...
						   		Date d = before.parse(monthlyReport.get(i).getDate().toString());	  
